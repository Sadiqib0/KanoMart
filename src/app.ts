...
const categoryLabels: Record<CategoryLabelKey, LocalizedString> = {
...
function inferDemandCategory(query: string, results: Product[]): string {
  if (results.length > 0) {
    return results[0].category.en;
  }

  const value = normalize(query);
  const dictionary: Array<{ category: string; terms: string[] }> = [
  ...
  const match = dictionary.find((entry) => entry.terms.some((term) => value.includes(term)));
  return match ? match.category : "Unmatched demand";
}

function saveSearch(query: string, results: Product[]): void {
...
function getCopy(en: string, ha: string): string {
  return state.language === "ha" ? ha : en;
}

function getLocalizedValue(value: LocalizedString | string): string {
...
function localizeCategory(category: string): string {
  ...
  if (value in categoryLabels) {
    return categoryLabels[value as CategoryLabelKey][state.language];
  }

  if (value.includes("Food") || value.includes("Abinci")) {
    return categoryLabels.Food[state.language];
  }

  if (value.includes("Children") || value.includes("Yara")) {
    return categoryLabels.Children[state.language];
  }

  if (value.includes("Essentials") || value.includes("Kayan yau da kullum")) {
    return categoryLabels.essentials[state.language];
  }

  if (value.includes("Fashion") || value.includes("Kaya")) {
    return categoryLabels.Fashion[state.language];
  }
...
function renderProductCard(product: Product): string {
  ...
  return `
    <article class="product-card">
      ...
      <p>${product.availability[state.language]}</p>
      <footer>
...
function updateResultCopy(query: string, results: Product[]): void {
...
function setLanguage(language: Language): void {
  ...
  document.title = getCopy("Kano Mart | Local Marketplace for Kano", "Kano Mart | Kasuwar Kano ta yanar gizo");
...
function groupByValue<T>(items: T[], accessor: (item: T) => string): Record<string, number> {
...
function sortEntries(grouped: Record<string, number>): Array<[string, number]> {
  return Object.entries(grouped).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]));
}

function renderRankList(container: HTMLElement, entries: Array<[string, number]>, emptyText: string): void {
  if (entries.length === 0) {
  ...
  container.innerHTML = entries
    .slice(0, 5)
    .map(([label, count]) => `<div class="rank-row"><strong>${label}</strong><span>${count}</span></div>`)
    .join("");
}

function renderDemandTrends(history: SearchRecord[]): void {
  ...
  elements.demandTrends.innerHTML = entries
    ...
    .map(([label, count]) => {
      ...
      return `
        <div class="trend-row">
          <div class="trend-label"><strong>${localizeCategory(label)}</strong><span>${count}</span></div>
...
function renderRecords(): void {
  ...
  elements.vendorPerformance.innerHTML = [...vendorRows, ...defaultVendors]
    .slice(0, 4)
    .map((row) => `<div class="record-row"><strong>${row.label}</strong><span>${getLocalizedValue(row.value)}</span></div>`)
    .join("");

  elements.orderRecords.innerHTML = demoOrders
    .map(
      (order) =>
        `<div class="record-row"><strong>${order.id}</strong><span>${getLocalizedValue(order.status)}</span></div>`
    )
    .join("");
  ...
  elements.paymentStatus.innerHTML = demoPayments
    .map(
      (payment) =>
        `<div class="record-row"><strong>${getLocalizedValue(payment.label)}</strong><span>${getLocalizedValue(
          payment.value
        )}</span></div>`
    )
    .join("");
...
function renderAdminDashboard(): void {
  ...
  renderRankList(elements.failedSearchList, failedPopular, getCopy("No failed searches yet.", "Babu binciken da ya gaza tukuna."));
          ...
          .map((item) => {
            ...
            return `
              <tr>
                ...
                <td>${localizeCategory(item.category)}</td>
                <td><span class="status-pill${failedClass}">${status}</span></td>
...
function exportSearchHistory(): void {
  const history = getStoredList<SearchRecord>(storageKeys.searches);
  const rows: Array<Array<string | number>> = [
    ["Query", "Language", "Results", "Category", "Status", "Time"],
    ...history.map((item) => [item.query, item.language, item.resultCount, item.category, item.status, item.createdAt]),
  ];
  const csv = rows
...
function clearPrototypeData(): void {
  const ok = window.confirm(
    getCopy("Clear saved prototype search and vendor data?", "A goge bayanan bincike da dillalai na gwaji?")
  );
...
elements.quickSearches.addEventListener("click", (event) => {
  ...
  const button = target?.closest<HTMLElement>("[data-query-en][data-query-ha]");
  if (!button) {
    return;
  }
