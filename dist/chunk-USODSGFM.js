...
var ADMIN_MOBILE_NUMBER = "08000000000";
var categoryLabels = {
...
// src/backend/vendors.ts
function normalizeVendorRequest(request) {
  return {
    ...request,
...
function getVendorRequests() {
  ...
  return [...getStoredList(storageKeys.liveVendors), ...getStoredList(storageKeys.vendors)].map(normalizeVendorRequest).filter((vendor) => {
    if (seen.has(vendor.id)) return false;
    seen.add(vendor.id);
    return true;
  });
}
function setLiveVendorRequests(nextVendors) {
  setStoredList(storageKeys.liveVendors, nextVendors);
}
function saveVendorRequest(input) {
  const request = {
    id: createId(),
...
function reviewVendorRequest(id, status, reviewNote = "") {
  const vendors = getVendorRequests();
  const vendor = vendors.find((item) => item.id === id);
  if (!vendor) return null;
  vendor.status = status;
  vendor.reviewedAt = (/* @__PURE__ */ new Date()).toISOString();
  vendor.reviewNote = reviewNote.trim();
  setStoredList(storageKeys.vendors, vendors);
  return vendor;
}
function getVendorStatusCounts(vendors = getVendorRequests()) {
...
function getUserProfiles() {
  return getStoredList(storageKeys.users).map((profile) => ({
    ...profile,
    phone: normalizePhone(profile.phone),
    role: profile.role === "vendor" ? "vendor" : "customer"
  }));
}
function findUserProfileByPhone(phone) {
...
function updateUserProfile(phone, input) {
  const existing = findUserProfileByPhone(phone);
...
function resetUserPassword(identifier, password) {
...
function createSessionForPhone(phone) {
  const normalized = normalizePhone(phone);
...
function loadSession() {
  try {
    return savedSession ? migrateSession(JSON.parse(savedSession)) : null;
  } catch {
    return null;
  }
}
