...
function localizeCategory(category) {
    ...
    if (value in categoryLabels) {
        return categoryLabels[value][state.language];
    }
...
function updateResultCopy(query, results) {
    ...
    elements.resultsIntro.textContent = getCopy(`Results related to "${query}" from trusted local vendors.`, `Sakamakon da ya shafi "${query}" daga amintattun dillalai.`);
    ...
    elements.resultStatus.textContent =
        results.length === 0
            ...
            : getCopy(`${results.length} result${results.length === 1 ? "" : "s"} found`, `An samu sakamako ${results.length}`);
}
...
function renderRecords() {
    ...
    elements.orderRecords.innerHTML = demoOrders
        .map((order) => `<div class="record-row"><strong>${order.id}</strong><span>${getLocalizedValue(order.status)}</span></div>`)
        .join("");
    elements.paymentStatus.innerHTML = demoPayments
        .map((payment) => `<div class="record-row"><strong>${getLocalizedValue(payment.label)}</strong><span>${getLocalizedValue(payment.value)}</span></div>`)
        .join("");
}
...
function clearPrototypeData() {
    const ok = window.confirm(getCopy("Clear saved prototype search and vendor data?", "A goge bayanan bincike da dillalai na gwaji?"));
    if (!ok) {
        return;
    }
...
function saveVendorRequest(event) {
    ...
    elements.vendorMessage.textContent = getCopy("Vendor request saved for admin review.", "An ajiye bukatar rajista domin admin ya duba.");
    renderAdminDashboard();
}
...
elements.quickSearches.addEventListener("click", (event) => {
    ...
    const button = target?.closest("[data-query-en][data-query-ha]");
    if (!button) {
        return;
    }
